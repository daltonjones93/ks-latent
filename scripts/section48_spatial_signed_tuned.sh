#!/bin/zsh
# User-directed 2026-09-01, "Section 48": same recipe family as Sections
# 46/47 (mlp/markovian aux, w_decorr=0, w_var_floor=0, NO delta_cap,
# --amp, Stage 2 warm-started, k_max=12, Stage 1 200ep/Stage 2 300ep),
# following Section 47's catastrophic collapse under spatial_signed at
# w_spatial=0.08 (min_eig=2.09e-6, cond#=1.07e7, only 2 effective dims):
# decrease w_var to 0.02 (from 0.04), set w_logdet to 0.005 (from 0.002,
# raised twice more from the user's own follow-up corrections), and
# spatial_signed's own weight down to 0.002 (from 0.08 -- a 40x reduction).
set -e
cd /Users/daltonjones/Documents/latent_DA

TAG=section48_mlpmarkovian_wvar002_wspatialsigned0002_logdet0005_200ep

echo "=== [1/3] Stage 1: mlp/markovian aux, w_decorr=0, w_var=0.02, w_spatial=0.002 (SIGNED), w_var_floor=0, w_logdet=0.005, NO delta_cap, --amp, 200 epochs ==="
mamba run -n da_env python scripts/train_stage1_patched.py \
  --profile full --encoder vit --aux-backbone mlp --mode markovian \
  --pos-encoding linear --attn-window 4 --token-window 16 \
  --w-decorr 0 --w-var 0.02 --w-spatial 0.002 --spatial-signed --w-var-floor 0 --w-logdet 0.005 \
  --full-propagator --amp \
