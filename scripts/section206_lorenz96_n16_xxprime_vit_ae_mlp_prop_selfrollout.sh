...
# correctly catching that "replace the encoder and decoder with the
# ViT" (their actual request) meant the AUTOENCODER only (this
# codebase's --encoder flag covers encoder+decoder together), not the
# PROPAGATOR (a separate --aux-backbone flag). Section 204's original,
# never-rescinded request was "encoder, decoder AND propagator be
# mlps" -- Section 205 should have kept --aux-backbone mlp and only
# changed --encoder. Fixed here: --encoder vit, --aux-backbone mlp.
#
# Everything else identical to Section 205: L96 N=16, F=6.0 (F=4.2 is
# NOT chaotic at this N, measured directly -- see Section 204/205's own
# headers), x+x' augmented 32-dim state, d_latent=8, mode=history
# (n_history=6), pos_encoding=linear, FULL attention (no --attn-window),
# same Stage-1 regularizers (w_var/w_spatial/w_logdet), same Stage-2
# stack (w_varmatch+w_spatial+w_logdet_rollout_latent) plus the new
# --w-spectrum-shape-self (this section's actual test, per
# docs/OPEN_QUESTIONS.md's "next step" recommendation).
#
# Verified via a real (--epochs 2) dry run of BOTH stages before this
# launch (vit-encoder + mlp-propagator + history, no attn-window: recon
# and propagator both train without error; Stage 2 with
# --w-spectrum-shape-self on: 2 epochs, no error) -- confirms this
# specific MIXED combination (encoder=vit, aux-backbone=mlp) -- not
# separately smoke-tested before Section 205's launch -- also trains
# cleanly.
set -e
cd /Users/daltonjones/Documents/latent_DA

TAG=section206_lorenz96_n16_xxprime_vit_ae_mlp_prop_selfrollout
DATASET=artifacts/datasets/lorenz96_trajectories_n16_f6.0_xxprime.h5
N_HISTORY=6

echo "=== [1/3] Stage 1: ViT encoder/decoder (NX=32, x+x', pos_encoding=linear, FULL attention -- no --attn-window), d_latent=8, mode=history (n_history=6) MLP-backbone propagator, w_var=0.02, w_spatial=0.01 signed, w_logdet=0.0035, on Lorenz-96 N=16 F=6.0 dt_snap=0.1, --amp, 40 epochs ==="
