#!/bin/zsh
# User-directed 2026-09-01, "Section 45": same recipe as Section 44 (the
# best result in the whole investigation, val_kmax_mse=0.0221) except
# w_spatial=0.14 (up from 0.1), Stage 1 epochs=240 (up from 120), Stage 2
# epochs=500 (up from 300). Everything else unchanged: w_decorr=0,
# w_var=0.03, w_var_floor=0, w_logdet=0.005, mlp/history(n_history=2)
# aux, NO delta_cap in either stage, Stage 2 warm-started from Stage 1's
# own aux, k_max=12.
set -e
cd /Users/daltonjones/Documents/latent_DA

TAG=section45_mlphistory2_wvar003_wspatial014_logdet0005_nodeltacap_240ep

echo "=== [1/3] Stage 1: mlp/history(n=2) aux, w_decorr=0, w_var=0.03, w_spatial=0.14, w_var_floor=0, w_logdet=0.005, NO delta_cap, 240 epochs ==="
mamba run -n da_env python scripts/train_stage1_patched.py \
  ...
  --w-decorr 0 --w-var 0.03 --w-spatial 0.14 --w-var-floor 0 --w-logdet 0.005 \
  --full-propagator \
  --epochs 240 --checkpoint-every 20 \
  --tag "$TAG" \
...
echo "=== [3/3] Stage 2: warm-started from Stage 1's own mlp/history(n=2) aux, NO delta_cap, k_max=12, 500 epochs ==="
STAGE2_TAG="${TAG}_warmstart_k12_500ep"
mamba run -n da_env python scripts/train_stage2_patched.py \
  --ae-checkpoint "$AE" \
  --init-prop-checkpoint "$AUX" \
  --epochs 500 --k-max 12 --k-warmup-epochs 353 --k-mid 8 --k-mid-epochs 294 \
  --checkpoint-every 50 \
  --tag "$STAGE2_TAG" \
  > artifacts/logs/stage2_${STAGE2_TAG}.log 2>&1

echo "=== Section 45 complete ==="
