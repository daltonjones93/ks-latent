#!/bin/zsh
# User-directed 2026-09-11, following Section 148 (local_field +
# masked_mlp_expand, attn_window=7, full pde_head spectrum -- confirmed
# healthy: D_KY=23.08/n_positive=12/lambda1=0.089, D3 significant,
# skill=1.31x): "I think the next experiment we should run should follow
# 148, but we should use a propagator with full attention window (still an
# mlp). I would like to penalize higher frequencies in the fourier
# transform of the latent space. This may help make the pde dynamics more
# easily fit and smooth."
#
# TWO changes from Section 148:
#
# 1. --aux-backbone masked_mlp_expand (attn_window=7) -> mlp (the FREE,
#    fully-dense propagator backbone -- "full attention window" -- same
#    backbone Sections 136/140/141/143 used, no attn_window/expand_factor
#    flags apply to it at all).
#
# 2. NEW regularizer --w-z-lowpass 1e-5 (Stage 1, on the encoder's own z)
#    and --w-z-lowpass-rollout 1e-4 (Stage 1 aux rollout AND Stage 2
#    propagator rollout) -- a brand-new mechanism built this session,
#    ks_latent.training.losses.latent_self_spectrum_lowpass_loss,
#    generalizing the existing (encoder_kind="spectral_field"-only)
#    low_pass_spectral_loss to ANY encoder's raw z: computes z's own
#    truncated self-FFT (encode_to_spectrum(z, K=d_latent//2+1=49), the
#    SAME "z's index as a periodic ring" convention spectral_pde_raw
#    already uses) and penalizes sum_k k * |z_hat_k|^2 -- literal
#    high-self-frequency energy in z's own spectrum. Motivated directly by
#    146-148: truncating what the pde_head is ALLOWED to see (--pde-K 24)
#    broke training; this instead gives the encoder/propagator an actual
#    gradient to make z's own self-spectrum smooth in the first place,
#    which should make ANY pde_head's fit easier/more stable without
#    needing an architectural truncation at all.
#
#    Weight choice: latent_self_spectrum_lowpass_loss's raw magnitude on
#    unit-variance random z(96) is ~7500 (power=1, K=49) -- unnormalized,
#    much larger than typical recon/regularizer loss scales in this
#    codebase (~1e-4 to ~1). 1e-5/1e-4 were chosen to bring its
#    CONTRIBUTION to the total loss into a comparable range (~0.01-0.1)
#    without dominating -- verified via a real dry run below, not just
#    computed on paper.
#
...
# --pde-poly-max-term-order 6 --pde-integrator euler (NO --pde-K --
# full spectrum), w_var=0.01/w_spatial=0.06 signed/w_logdet=0.01/
...
# stages before this launch -- --aux-backbone mlp + --w-z-lowpass/
# --w-z-lowpass-rollout all accepted, losses compute and decrease
# normally, no errors, and the new term's contribution to the total loss
# stays a small fraction of it (not dominating).
#
...
TAG=section149_localfield_mlp_zlowpass_200ep

echo "=== [1/6] Stage 1: local_field + FREE mlp propagator (full attention) + --pde-distill --pde-mutual (NO --pde-K -- full spectrum, K=d_latent//2+1=49) --pde-poly-max-term-order 6, w_var=0.01/w_spatial=0.06/w_logdet=0.01/w_smooth=0.006/w_pde_distill=0.5, NEW w_z_lowpass=1e-5/w_z_lowpass_rollout=1e-4, --amp, 200 epochs ==="
mamba run -n da_env python scripts/train_stage1_patched.py \
  ...
  --w-z-lowpass 1e-5 --w-z-lowpass-rollout 1e-4 \
  --full-propagator --amp \
