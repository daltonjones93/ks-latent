#!/bin/zsh
# User-directed 2026-09-05, "Section 95": "can we run 94 again, but let's
# have w_spatial .03, and w_smooth .0015." (Refined from an initial
# proposal of w_spatial=.028/w_smooth=.008 after a reasonableness check:
# w_smooth=.008 would have been a 16x jump from 94's 0.0005, pushing its
# loss contribution to ~5x l_recon's own magnitude -- likely dominant/
# collapse-risking, per temporal_smoothness_loss's own documented
# collapse-risk docstring. w_smooth=.0015 instead contributes ~0.000113,
# in line with l_recon's own scale (0.000122) rather than dominating it.)
#
# Identical to Section 94 (scripts/section94_scaled_60pct.sh) in every
# respect EXCEPT the two regularizer weights:
#   - w_spatial (signed): 0.019 -> 0.03.
#   - w_smooth: 0.0005 -> 0.0015 (3x).
#
# Architecture/propagator UNCHANGED from 94: vit_fourier_hybrid,
# d_model=64/fourier_hidden=95/fourier_blocks=2 (~516K total, 75.3%/24.7%
# ViT/Fourier, ~61% of Section 93's size), pool=token_mlp/dec_pool=
# token_mlp (reduction=8/hidden=128), enc_out_modes=8, dec_fno_modes=12,
# attn_window=4/pos_encoding=linear/token_window=16, Section 52's EXACT
# mlp/markovian propagator (hidden=128/n_blocks=3). w_var=0.02,
# w_var_floor=0, w_logdet=0.008, NO lambda_z, w_decorr=0 unchanged.
#
# Context: Section 94 (=93 scaled to 60%) plateaued rather than improved
# further on val_kmax_mse (0.0123 vs 93's 0.0118) and smoothness (both
# statistically flat vs 93) -- this tests whether pushing the two
# regularizers that specifically target smoothness/structure (w_spatial,
# w_smooth) further, at 94's already-plateaued size, can move the needle
# where pure size reduction stopped helping.
#
# Verified this turn: a real 4-epoch Stage 1 smoke run (val_recon_final=
# 0.024883, no errors) and a 4-epoch Stage 2 warm-start smoke run (both
