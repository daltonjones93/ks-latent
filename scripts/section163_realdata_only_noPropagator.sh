#!/bin/zsh
# User-directed 2026-09-12. Sections 153/160/161/162 all fit pde_head's
# DOMINANT signal (w_pde_distill=0.5, mutual) against the PROPAGATOR's
# (aux's) own predicted next state, NOT against the real encoder's own
# dz/dt. User caught this directly: "are we trying to fit the pde using
# the propagator, or the latent states? I think we should use the data
# solely when it's passed through the encoder to estimate dz/dt, I don't
# think the propagator should be used at all." Confirmed by inspection:
# Section 162 (killed mid-Stage-1 to launch this instead) had ZERO
# real-data fitting signal in Stage 1 (only w_pde_distill=0.5 --pde-mutual
# against the propagator) and a MIX in Stage 2 (w_pde_distill=0.5 vs.
# propagator + the new w_pde_distill_real_rollout=0.3 vs. real data).
#
# This is a real confound, not just a style preference: fitting against
# the propagator's own predictions tests "can pde_head mimic whatever the
# propagator learned," not "does a local PDE explain the REAL KS latent
# dynamics" -- and because it's MUTUAL, it also pressures the propagator
# itself toward whatever dynamics pde_head can already express, the same
# "pressure toward simplicity" mechanism behind this project's own H-PROP
# fixed-point-collapse finding.
#
# THIS section removes --w-pde-distill/--pde-mutual ENTIRELY from BOTH
# stages (pde_head no longer ever sees the propagator's own predictions).
# --pde-distill is still passed (builds the pde_head object), but its
# ONLY fitting signal is now real, encoder-derived (z_t, z_{t+1}, ...)
# transitions:
#   Stage 1: --w-pde-distill-real 0.5 (single-step, every consecutive
#     REAL pair in the training window -- replaces the removed
#     propagator-matching term at the same weight).
#   Stage 2: --w-pde-distill-real 0.5 (single-step, consistent with Stage
#     1) + --w-pde-distill-real-rollout 0.3 --pde-distill-real-rollout-k 4
#     (multi-step, the iLED L_forecast analogue -- unchanged from 160-162).
# Both real-data terms use pde_distill_real_detach=True (default) --
# gradient reaches ONLY pde_head's own parameters, never the encoder or
# propagator, so pde_head is now a purely passive analysis of the real
# latent dynamics, fully decoupled from whatever the propagator is doing.
# --w-channel-mean 0.01 (Stage 1) and --w-pde-nonlinear-l2 0.01 (Stage 2
# only, matching 162's own placement) unchanged from 162.
#
# Verified via a real (--profile full, --epochs 3) dry run of both stages
# with this exact recipe -- no NaN, pde_distill_real/pde_distill_real_
# rollout/pde_nonlinear all print and behave sensibly, confirmed pde_head
# receives gradient with NO w_pde_distill active.
#
...
# as Sections 153/160/161/162 -- directly comparable numbers.
set -e
cd /Users/daltonjones/Documents/latent_DA

TAG=section163_realdata_only_noPropagator

echo "=== [1/6] Stage 1: Section 153's recipe + --w-channel-mean 0.01, pde_head fit SOLELY against real encoder-derived transitions (--w-pde-distill-real 0.5, NO --w-pde-distill/--pde-mutual -- propagator never seen): local_field + masked_mlp_expand (attn_window=7, expand_factor=3) + --pde-distill (builds pde_head only) (NO --pde-K -- full spectrum, K=d_latent//2+1=49) --pde-poly-max-term-order 6, w_var=0.01/w_spatial=0.06/w_logdet=0.01/w_smooth=0.006, --amp, 200 epochs ==="
...
echo "=== [2/6] Stage 2: propagator trains independently (pde_head's loss no longer touches it at all); pde_head continues fitting SOLELY against real data: --w-pde-distill-real 0.5 (single-step) + --w-pde-distill-real-rollout 0.3 --pde-distill-real-rollout-k 4 (multi-step, iLED L_forecast) + --w-pde-nonlinear-l2 0.01 (iLED L_non-linearity), NO --w-pde-distill, grad_clip active, --amp, k_max=12, 300 epochs (established curriculum) ==="
