#!/bin/zsh
# User-directed 2026-09-01, "Section 50": same recipe as Sections 48/49
# (mlp/markovian aux, w_var=0.02, w_decorr=0, w_var_floor=0, NO delta_cap,
# --amp, Stage 2 warm-started, k_max=12, Stage 1 200ep/Stage 2 300ep),
# but w_spatial (signed)=0.0065 (up from 0.004) and w_logdet=0.0035
# (down slightly from 0.004).
set -e
cd /Users/daltonjones/Documents/latent_DA

TAG=section50_mlpmarkovian_wvar002_wspatialsigned0065_logdet0035_200ep

echo "=== [1/3] Stage 1: mlp/markovian aux, w_decorr=0, w_var=0.02, w_spatial=0.0065 (SIGNED), w_var_floor=0, w_logdet=0.0035, NO delta_cap, --amp, 200 epochs ==="
mamba run -n da_env python scripts/train_stage1_patched.py \
  --profile full --encoder vit --aux-backbone mlp --mode markovian \
  --pos-encoding linear --attn-window 4 --token-window 16 \
  --w-decorr 0 --w-var 0.02 --w-spatial 0.0065 --spatial-signed --w-var-floor 0 --w-logdet 0.0035 \
  --full-propagator --amp \
