...
# contributes zero).
#
# Section 165 (mean-conservation SOFT penalty, weight 200) + Section 166
# (also removing --w-spatial/--w-smooth) both regressed standalone
# stability vs. Section 164 (166: diverges ~step 29, 9/10 mixing-check
# ICs diverged) rather than fixing it. User asked directly: "could mean
# conservation cause pde divergence?" -- answer: plausibly yes, for three
# reasons discussed: (1) the weight (200) was heuristically calibrated,
# easily too strong; (2) the constraint (field(z).mean(dim=-1)=0) is
# UNDER-DETERMINED -- many different coefficient changes can satisfy it,
# with no guarantee the optimizer picks the "good" one (shrinking w_x*w_x
# specifically) rather than a differently-harmful one; (3) it is a SOFT,
# data-dependent match on the training batch, not a hard structural
# invariant -- satisfying it on real on-attractor states does not
# guarantee it holds on the off-distribution states a free rollout visits.
#
# Worked out the EXACT general rule instead (user: "please try to
# exclude every even-combined-order two-factor term from the library"):
# for a two-factor product term d_i*d_j, repeated integration by parts
# shows its spatial mean is EXACTLY zero for ANY state when i+j is odd
# (reduces to an exact total derivative, e.g. w_xx*w_xxx = d/dx(w_xx^2/2)
# -- matches w*w_x, the true KS nonlinearity itself, i+j=0+1=1, odd), and
# generically NONZERO when i+j is even (reduces to a perfect square
# (w^((i+j)/2))^2 -- matches w_x*w_x, the largest measured violator at
# -0.043, i+j=1+1=2). Implemented as `spectral_poly_exclude_
# nonconservative` (ks_latent/models/propagator.py, `_poly_weight()`) --
# architecturally zeros every even-combined-order length-2 term's raw
# contribution, EXACTLY, for every state, with NO weight to tune (unlike
# the failed soft penalty). Verified directly via unit test: gradient at
# every even-2-factor position (w*w, w_x*w_x, w*w_xx, w_x*w_xxx, ...) is
# exactly 0.0, every odd-2-factor and linear position remains fully free.
# Requires poly_degree<=2 (raises otherwise -- the rule isn't derived for
# length>=3 terms); this recipe already uses poly_degree=2.
#
# ONE change from Section 165: --w-pde-mean-conservation 200.0 REPLACED
# by --pde-poly-exclude-nonconservative (Stage 1 only -- an architecture-
# construction-time flag; Stage 2 loads pde_head's architecture from the
# Stage-1 checkpoint, so nothing to set there). --pde-poly-no-constant
# and the doubled --w-spatial 0.12/--w-smooth 0.012 are KEPT (per this
# section's own postmortem of 166: removing spatial/smooth made things
# worse, so they stay in while only the mean-conservation mechanism
# changes -- a clean, single-variable swap).
#
# Verified via a real (--profile full, --epochs 3) dry run of Stage 1
# with this exact recipe -- no NaN, trains normally.
#
...
TAG=section167_excludenonconservative

echo "=== [1/7] Stage 1: Section 165's recipe, --w-pde-mean-conservation REPLACED by NEW --pde-poly-exclude-nonconservative (architecturally zeros every even-combined-order two-factor term -- exact, no weight to tune): local_field + masked_mlp_expand (attn_window=7, expand_factor=3), --pde-poly-no-constant, --w-spatial 0.12 --w-smooth 0.012 (kept per 166's postmortem), pde_head fit SOLELY against real encoder-derived transitions, MUTUAL (--w-pde-distill-real 0.5 --pde-distill-real-mutual -- gradient reaches the encoder; NO --w-pde-distill/--pde-mutual -- propagator never seen) (NO --pde-K -- full spectrum, K=d_latent//2+1=49) --pde-poly-max-term-order 6, w_var=0.01/w_logdet=0.01, --amp, 200 epochs ==="
