#!/bin/zsh
# User-directed 2026-09-02, "Section 55": same larger models as Sections
# 53/54 (ViT AE d_model=108 ~1.24x, mlp/markovian aux hidden=141/n_blocks=3
# ~1.2x), same regularizer weights as Section 52 (w_decorr=0, w_spatial
# (signed)=0.01, w_var_floor=0, NO delta_cap, --amp, Stage 2 warm-started,
# k_max=12, Stage 1 200ep/Stage 2 300ep) EXCEPT w_var changed 0.02 ->
# 0.015 and w_logdet changed 0.0035 -> 0.003 (this is the third attempt at
# Section 55 -- two earlier weight combos, 0.002/0.00033 and
# 0.00225/0.00036, were each killed before Stage 1 finished, per user
# request, before landing on this pair).
set -e
cd /Users/daltonjones/Documents/latent_DA

TAG=section55_mlpmarkovian_bigger12x_dmodel108_wvar0015_wspatialsigned01_logdet0003_200ep

echo "=== [1/3] Stage 1: ViT AE (d_model=108, ~1.24x) + mlp/markovian aux (hidden=141/n_blocks=3, 1.2x), w_decorr=0, w_var=0.015, w_spatial=0.01 (SIGNED), w_var_floor=0, w_logdet=0.003, NO delta_cap, --amp, 200 epochs ==="
mamba run -n da_env python scripts/train_stage1_patched.py \
  ...
  --w-decorr 0 --w-var 0.015 --w-spatial 0.01 --spatial-signed --w-var-floor 0 --w-logdet 0.003 \
  --full-propagator --amp \
