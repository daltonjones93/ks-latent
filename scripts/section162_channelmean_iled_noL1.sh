#!/bin/zsh
# User-directed 2026-09-12. Section 160 (Section 153's recipe + the two
# new iLED terms) trained cleanly but the GIF of pde_head's own free-
# running rollout still showed a strong, near-sinusoidal single-frequency
# pattern. Measured directly: the REAL encoded z0 (before pde_head ever
# touches it) already has 78% of its self-FFT energy at k=0 (mean) and
# 20.5% at EXACTLY k=32=d_latent/local_channels=96/3 -- everything else
# combined is <1%. Per-channel means across sites: channel 0 (anchor) ~0
# as expected, channels 1/2 (free residual) drifted to ~9.10/~7.11 -- the
# same period-local_channels flattening artifact `--w-channel-mean` fixes.
#
# Section 161 combined channel-mean + the iLED terms + Section 153's own
# `--w-pde-coeff-l1 0.05 --pde-coeff-l1-linear-only` (carried over
# unchanged, on the premise that it was needed to prevent the periodic
# degenerate solution). That premise was WRONG, caught directly by the
# user: 153's own saved smooth-field GIF/PNG (docs/figures/pde_smooth_
# hovmoller_section153_..._300ep.png) shows the EXACT SAME static,
# frozen vertical-stripe pattern as Section 160's, despite having all 5
# linear coefficients driven to zero by that same L1 penalty. L1-linear-
# only zeroes the fitted PDE's own linear COEFFICIENTS; it does nothing
# about the INPUT's own spectral concentration (the channel-mean
# artifact), which is the actual, measured cause of the frozen pattern --
# a nonlinear-dominated closure evaluated on an input confined to two
# modes just keeps reinforcing those same two modes. So L1-linear-only
# was never doing the periodicity-prevention job credited to it, and
# keeping it active in this test would confound whether any result comes
# from iLED's own mechanisms or from L1 still doing whatever it does.
#
# THIS section drops --w-pde-coeff-l1/--pde-coeff-l1-linear-only entirely
# (killed Section 161 mid-Stage-1 to launch this instead) -- isolates
# whether the channel-mean fix + iLED's own two mechanisms alone
# (nonlinear-closure-magnitude penalty + real-data rollout distillation,
# with NO artificial suppression of linear terms) are sufficient to keep
# pde_head's fit/stability intact once the mode-32 artifact is removed --
# a cleaner test of iLED's own claim (their paper uses no L1-style term).
# Everything else identical to Section 161/160: --w-channel-mean 0.01,
# --w-pde-distill-real-rollout 0.3 --pde-distill-real-rollout-k 4,
# --w-pde-nonlinear-l2 0.01, grad clipping active.
#
# Verified via a real (--profile full, --epochs 3) dry run of Stage 1
# (no --w-pde-coeff-l1) then Stage 2's exact recipe warm-started from
# that checkpoint -- no NaN, pde_nonlinear/pde_distill_real_rollout both
# print and behave sensibly.
#
...
# as Sections 153/160/161 -- directly comparable numbers.
set -e
cd /Users/daltonjones/Documents/latent_DA

TAG=section162_channelmean_iled_noL1

echo "=== [1/6] Stage 1: Section 153's recipe + --w-channel-mean 0.01 (Section 155's mode-32 artifact fix), NO --w-pde-coeff-l1: local_field + masked_mlp_expand (attn_window=7, expand_factor=3) + --pde-distill --pde-mutual (NO --pde-K -- full spectrum, K=d_latent//2+1=49) --pde-poly-max-term-order 6, w_var=0.01/w_spatial=0.06/w_logdet=0.01/w_smooth=0.006/w_pde_distill=0.5, --amp, 200 epochs ==="
...
echo "=== [2/6] Stage 2: propagator AND pde_head continue together, UNFROZEN (mutual), w_pde_distill=0.5, NO w_pde_coeff_l1 + NEW --w-pde-distill-real-rollout 0.3 --pde-distill-real-rollout-k 4 (iLED L_forecast) + --w-pde-nonlinear-l2 0.01 (iLED L_non-linearity), grad_clip now active (general fix), --amp, k_max=12, 300 epochs (established curriculum) ==="
