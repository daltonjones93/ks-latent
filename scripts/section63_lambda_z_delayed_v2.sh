#!/bin/zsh
# User-directed 2026-09-02, "Section 63": follow-up to Section 62
# (lambda_z=0.0005, active from epoch 30) -- "wow, 62 collapsed to the
# same latent state as before. That's sort of remarkable. run 63 with the
# same params as 62 except lambda_z = .001." Section 62 did NOT show the
# textbook collapse signature in the covariance spectrum (min_eig=6.76e-6,
# not near machine epsilon), but the user observed the latent_state_
# evolution GIF visually settling into a repeating/near-constant pattern
# over time -- consistent with BandedSmoothness's documented degenerate
# optimum (every channel -> the same constant value) starting to bite,
# just not yet catastrophically in the raw eigenvalue spectrum. This run
# doubles lambda_z (0.0005 -> 0.001) to see whether that visual collapse
# becomes more pronounced/is confirmed in the spectrum too. lambda_z alone
# (NO lambda_decorr) remains a DELIBERATE choice, not an oversight -- see
# Section 62's script for the full rationale. --reg-start-epoch 30
# unchanged: the AE trains completely unconstrained by this term for the
# first 30 epochs, then it activates at full weight (no ramp) from epoch
# 30 on. Otherwise identical to Section 52's recipe (default-size vit AE +
...
TAG=section63_mlpmarkovian_wvar002_wspatialsigned01_logdet0035_lambdaz0001after30ep_200ep

echo "=== [1/3] Stage 1: mlp/markovian aux, w_decorr=0, w_var=0.02, w_spatial=0.01 (SIGNED), w_var_floor=0, w_logdet=0.0035, lambda_z=0.001 (active from epoch 30), NO delta_cap, --amp, 200 epochs ==="
mamba run -n da_env python scripts/train_stage1_patched.py \
  ...
  --lambda-z 0.001 --reg-start-epoch 30 \
  --full-propagator --amp \
