#!/bin/zsh
# User-directed 2026-09-11. Sections 146 (attn_window=7, --pde-K 24) and
# 147 (attn_window=40 -- effectively dense/unmasked, --pde-K 24 still on)
# BOTH stalled in Stage 2 with essentially IDENTICAL numbers (val_kmax_mse
# stuck ~0.42-0.46, pde_distill stuck ~1.09, vs. Section 145's healthy
# converged 0.073/0.030) -- since removing the masking entirely (147)
# didn't fix it, this isolates --pde-K 24 (the pde_head's own self-FFT
# truncation to the lowest half of z's spectrum) as the actual cause, not
# attn_window. User: "interesting. it appears the truncation of the pde
# modes contributes to the failure. kill 147, and run 148 as the same as
# 146 without truncating the fourier basis."
#
# ONE change from Section 146: --pde-K 24 REMOVED (reverts to the default,
# d_latent//2+1=49, i.e. NO truncation -- the pde_head sees the entire
# rFFT spectrum of z's own d_latent=96-length ring again, same as Section
# 145). --prop-attn-window stays at 7 (kept from 146, "the same as 146
# without truncating the fourier basis").
#
# This isolates the remaining question directly: 145 (window=8, full
# spectrum) trained healthily; 146 (window=7, HALF spectrum) stalled; 147
# (window=40/effectively dense, HALF spectrum) also stalled. If 148
# (window=7, FULL spectrum) trains healthily, that confirms --pde-K
# truncation itself is what breaks Stage 2's joint mutual training here
# (plausibly: halving K makes pde_head's own fit systematically worse,
# and since this is a MUTUAL loss -- gradient flows back into the
# propagator too -- a badly-fit pde_head target could be actively
# destabilizing the propagator during Stage 2, not just failing to help
# it).
#
...
# masked_mlp_expand as the SOLE primary propagator (attn_window=7,
# expand_factor=3), --pde-distill --pde-mutual --pde-field-kind polynomial
...
# mutual continuation, w_pde_distill=0.5, k_max=12, 300 epochs (established
# curriculum).
#
# Verified via a real (--profile full, 2-epoch) dry run through BOTH
# stages before this launch -- pde_distill loss computes and decreases
# normally with attn_window=7 and no --pde-K override, no errors.
#
...
# as Sections 145/146/147 -- directly comparable numbers.
set -e
cd /Users/daltonjones/Documents/latent_DA

TAG=section148_localfield_maskedmlpexpand_w7_nopdeK_200ep

echo "=== [1/6] Stage 1: local_field + masked_mlp_expand (attn_window=7, expand_factor=3) + --pde-distill --pde-mutual (NO --pde-K -- full spectrum, K=d_latent//2+1=49) --pde-poly-max-term-order 6, w_var=0.01/w_spatial=0.06/w_logdet=0.01/w_smooth=0.006/w_pde_distill=0.5, --amp, 200 epochs ==="
