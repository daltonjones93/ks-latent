...
# contributes zero).
#
# TWO changes from Section 164 (fix) plus ONE separate user-requested
# change (regularizer strength):
#   1. NEW --pde-poly-no-constant (both stages, via pde_head config --
#      architecturally zeros the bias/intercept term exactly, one of the
#      two measured sources).
#   2. NEW --w-pde-mean-conservation 200.0 (both stages) -- the new
#      general mechanism (ks_latent.training.loops.pde_head_field_output,
#      see its docstring) directly penalizing field(z).mean(dim=-1)^2 on
#      real detached states, targeting whatever combination of terms
#      (e.g. w_x*w_x, w*w) violates the conservation law, rather than
#      hand-zeroing specific coefficients. Weight chosen empirically via
#      dry run (the underlying quantity is naturally tiny, ~(0.0045)^2,
#      so a much larger weight than other regularizers is needed for it
#      to contribute a comparable gradient -- untested at this exact
#      value, verify the printed pde_mean trend during training).
#   3. User's SEPARATE request, unrelated to the drift fix: DOUBLE
#      --w-spatial (0.06 -> 0.12) and --w-smooth (0.006 -> 0.012) in
#      Stage 1 (the only stage that uses them in this recipe).
# Everything else identical to Section 164 (channel-mean fix, no L1,
...
# in Stage 2).
#
# Verified via a real (--profile full, --epochs 3) dry run of both
# stages with this exact recipe -- no NaN, pde_mean prints and is a
# small but nonzero, sensibly-scaled quantity relative to the other
# pde_head loss terms.
#
...
TAG=section165_meanconservation

echo "=== [1/7] Stage 1: Section 164's recipe + NEW --pde-poly-no-constant (zeros the bias term) + NEW --w-pde-mean-conservation 200.0 (penalizes field(z)'s spatial mean) + DOUBLED --w-spatial 0.12 --w-smooth 0.012 (user request): local_field + masked_mlp_expand (attn_window=7, expand_factor=3), pde_head fit SOLELY against real encoder-derived transitions, MUTUAL (--w-pde-distill-real 0.5 --pde-distill-real-mutual -- gradient reaches the encoder; NO --w-pde-distill/--pde-mutual -- propagator never seen) (NO --pde-K -- full spectrum, K=d_latent//2+1=49) --pde-poly-max-term-order 6, w_var=0.01/w_logdet=0.01, --amp, 200 epochs ==="
