#!/bin/zsh
# User-directed 2026-09-02, "Section 64": w_logdet=0.01 (up from 0.0035),
# w_spatial(signed)=0.02 (up from 0.01), lambda_z=0.0075 (up from
# Section 63's 0.001, active from epoch 30 same as 62/63) -- pushing all
# three anti-collapse/coherence counterweights up together, to see
# whether a strong logdet barrier can support a much more aggressive
# lambda_z (directly testing the user's hypothesis that "the logdet term
# should prevent it from collapsing too much"). w_var stays at Section
# 52's 0.02 (not mentioned as changed). lambda_decorr stays 0 (still
# lambda_z alone, deliberate, per Section 62/63's rationale). Otherwise
# identical to Section 52's recipe (default-size vit AE + mlp/markovian
# aux, w_decorr=0, w_var_floor=0, NO delta_cap, --amp, Stage 2
# warm-started, k_max=12, Stage 1 200ep/Stage 2 300ep).
set -e
cd /Users/daltonjones/Documents/latent_DA

TAG=section64_mlpmarkovian_wvar002_wspatialsigned002_logdet001_lambdaz00075after30ep_200ep

echo "=== [1/3] Stage 1: mlp/markovian aux, w_decorr=0, w_var=0.02, w_spatial=0.02 (SIGNED), w_var_floor=0, w_logdet=0.01, lambda_z=0.0075 (active from epoch 30), NO delta_cap, --amp, 200 epochs ==="
