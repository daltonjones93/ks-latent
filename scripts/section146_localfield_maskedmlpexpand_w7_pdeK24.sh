#!/bin/zsh
# User-directed 2026-09-11, following Section 145's confirmed genuine-chaos
# recovery on masked_mlp_expand (D_KY=23.08/n_positive=12/lambda1=0.088,
# D3 significant, all matching free-mlp Section 141 almost exactly -- see
# that section's own coefficient-consistency check) but weak DA skill
# (1.13x, vs 141/143's 1.84-2.07x) and a standalone pde_head divergence at
# t=73: "I would like to run section 146 with the same settings as 145,
# except decrease attn_window for the propagator by one. I would also like
# the pde_head to only use .5 of the frequencies z, the lowest frequencies.
# I suspect this might have a stabilizing effect so it's not fitting for so
# much noise."
#
# TWO changes from Section 145, everything else IDENTICAL:
#
# 1. --prop-attn-window 8 -> 7 (masked_mlp_expand's propagator, effective
#    COMPOSED receptive field = 3*attn_window across its fixed 3-layer
#    stack, so this drops from 24 -> 21 -- still comfortably inside the
#    ~20-30 target range Section 141's own D3 coupling-mass measurement
#    identified, just slightly tighter).
#
# 2. --pde-K 24 (pde_head's own self-FFT truncation, AuxPropagatorConfig.
#    spectral_K under backbone='spectral_pde_raw'). Default (used by every
#    prior --pde-distill run this arc, including 145) is d_latent//2+1=49,
#    i.e. NO truncation -- the pde_head sees the ENTIRE rFFT spectrum of
#    z's own d_latent=96-length ring. 24 is ~half of that (floor(49/2)),
#    keeping only the LOWEST 24 frequency bins and discarding the upper
#    half -- a genuine low-pass bandlimit on what the pde_head's derivative
#    synthesis (`encode_to_spectrum`/`synthesize_derivatives`) can see.
#    User's hypothesis: the pde_head may be fitting noise in the high-
#    frequency part of z's own self-spectrum (plausible given z is a
#    LEARNED ordering with no guaranteed smoothness the way real KS's own
#    spectrum has), and restricting it to the lowest half should have a
#    stabilizing effect. This is a pde_head-only change -- the propagator
#    itself still sees/produces the FULL d_latent=96 vector; only the
#    pde_head's own internal self-FFT truncation changes.
#
# Otherwise IDENTICAL to Section 145: local_field (n_sites=32/
...
# --pde-poly-max-term-order 6 --pde-integrator euler, w_var=0.01/
...
# stages before this launch -- --pde-K 24 accepted, pde_distill loss
# computes and decreases normally with attn_window=7, no errors.
#
...
# as Section 145 -- directly comparable numbers.
set -e
cd /Users/daltonjones/Documents/latent_DA

TAG=section146_localfield_maskedmlpexpand_w7_pdeK24_200ep

echo "=== [1/6] Stage 1: local_field + masked_mlp_expand (attn_window=7, expand_factor=3) + --pde-distill --pde-mutual --pde-K 24 (half spectrum, lowest freqs only) --pde-poly-max-term-order 6, w_var=0.01/w_spatial=0.06/w_logdet=0.01/w_smooth=0.006/w_pde_distill=0.5, --amp, 200 epochs ==="
