...
#
# Correctness fix (Section 198 itself): the Lyapunov spectrum measurement
# uses mode='two_step' (the library's EXACT tangent map on the true
# 2*d_latent-dimensional (z_{n-1},z_n) phase space) whenever the
# propagator itself is mode=two_step -- 'single_state' (ks_latent.
# analysis.lyapunov's own "the z->z approximation") is only correct for a
# genuinely Markovian propagator.
#
# FOURTH follow-up (this section, 199): "I think 198 did better with a
# markovian propagator. can you run it again with this. kill any current
# training. call this 199." Directly confirmed before switching: Section
# 198's own Stage-1 two_step propagator measured lambda1=0.0102,
# n_positive=1/20, D_KY=2.01 -- WEAKER than both the true L96 N=64/F=4.2
# reference (0.080/5/11.3) and Section 196's own local_field+markovian
# result (0.056/3/4.93). --mode markovian REPLACES two_step (--aux-
# backbone mlp restored -- markovian, unlike two_step, actually reads it);
# Lyapunov measurement reverts to mode='single_state' (correct for a
# genuinely Markovian propagator). Everything else identical to 198
# (Section 52's ViT recipe, d_latent=20, 40/60-epoch schedule, same
# dataset).
set -e
cd /Users/daltonjones/Documents/latent_DA

TAG=section199_lorenz96_section52_markovian
DATASET=artifacts/datasets/lorenz96_trajectories_n64_f4.2.h5

echo "=== [1/3] Stage 1: Section 52's recipe (ViT encoder, pos_encoding=linear, attn_window=4, token_window=16, d_latent=20) + mode=markovian dense MLP propagator, w_var=0.02, w_spatial=0.01 signed, w_logdet=0.0035, NO delta_cap, on Lorenz-96 N=64 F=4.2 dt_snap=0.1, --amp, 40 epochs (1/5 of KS's own 200 -- this dataset has 8x more snapshots/trajectory, so 40 epochs here covers more gradient updates than 200 would on KS) ==="
