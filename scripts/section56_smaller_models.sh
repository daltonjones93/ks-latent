#!/bin/zsh
# User-directed 2026-09-02, "Section 56": same training settings as
# Section 52 (mlp/markovian aux, w_decorr=0, w_var=0.02, w_spatial=0.01
# (SIGNED), w_var_floor=0, w_logdet=0.0035, NO delta_cap, --amp, Stage 2
# warm-started, k_max=12, Stage 1 200ep/Stage 2 300ep), but with the ViT
# encoder/decoder AND the aux/Stage-2 propagator each ~20% SMALLER
# (mirror image of Section 53's +20% scaling):
#   --d-model 84: encoder/decoder d_model=84 (n_heads=4/n_blocks=3/
#     mlp_ratio=4 unchanged) gives 641730 total params vs. the d_model=96
#     default's 816342 (ratio 0.7861, closest achievable to 0.8x with
#     d_model divisible by n_heads=4 -- checked directly by instantiating
#     ViTAutoencoderConfig/build_autoencoder at d_model in {64..96}).
#   --aux-hidden 114 --aux-blocks 3: propagator hidden=114/n_blocks=3
#     gives 89762 params vs. the hidden=128 default's 111532 (ratio
#     0.8048, closest achievable to 0.8x -- checked directly by
#     instantiating AuxPropagatorConfig/AuxPropagator at hidden in
#     110..119).
set -e
cd /Users/daltonjones/Documents/latent_DA

TAG=section56_mlpmarkovian_smaller08x_dmodel84_wvar002_wspatialsigned01_logdet0035_200ep

echo "=== [1/3] Stage 1: ViT AE (d_model=84, ~0.79x) + mlp/markovian aux (hidden=114/n_blocks=3, ~0.80x), w_decorr=0, w_var=0.02, w_spatial=0.01 (SIGNED), w_var_floor=0, w_logdet=0.0035, NO delta_cap, --amp, 200 epochs ==="
