#!/bin/zsh
# User-directed 2026-09-10: after Section 144 (local_field + masked_mlp_expand,
# attn_window=8, effective receptive field 24, NO pde_head -- isolating whether
# the properly-sized masked propagator could sustain chaos on its own) was
# caught mid-training and diagnosed directly: "check the propagator for signs
# of chaos, if you don't find them, kill this training and add pde-distill
# using the same exact setup as 141."
#
# Direct measurement on 144's own Stage-1 checkpoint at epoch 119 AND epoch
# 139 (two successive checkpoints, to see the TREND, not just one snapshot):
# max|z| over a 60-step rollout grew 5.7 -> 9.0 -> 16.0 -> 27.6 -> 48.4 ->
# 104.0 -> 127.2 (epoch 139 checkpoint), with cross-sample z.std growing
# monotonically 0.73 -> 5.5 over the same 60 steps -- NOT the bounded-
# separation/O(1)-cross-sample-std signature every genuinely chaotic run in
# this arc (136/140/141/143) settles into. This is the same unbounded-
# blowup signature Section 142 showed (confirmed on RE-check to be
# worsening, not self-correcting) -- divergence, not chaos. Killed per
# direct instruction rather than let it run to completion.
#
# This run: IDENTICAL propagator config to Section 144 (local_field
# n_sites=32/local_channels=3/site_mix_radius=2/n_site_mix_layers=3/hidden=32
# + masked_mlp_expand as the SOLE primary propagator, attn_window=8,
# expand_factor=3 -> effective composed receptive field 24, sized off
# Section 141's own D3 coupling measurement), but with --pde-distill added
# using Section 141's EXACT recipe (same regularizer weights, same
# pde_field_kind=polynomial/degree=2/max_term_order=6/integrator=euler,
# same --pde-mutual, same Stage-2 w_pde_distill=0.5 continuation, same
# k-curriculum) -- the only variable changed from 141 is the propagator
# backbone itself (mlp -> masked_mlp_expand). Tests whether a genuinely
# local/masked propagator, once regularized/distilled toward PDE-describable
# dynamics the same way 141's free mlp was, can be pulled into a stable,
# bounded regime instead of diverging on its own.
#
# Verified via a real (--profile full, 2-epoch) dry run through BOTH stages
# before this launch -- pde_distill loss computes and decreases normally
# with the masked_mlp_expand backbone, no errors.
#
# Same before/after Jacobian-spectrum+conditioning check, visualization,
# Gate 3/4 suite, and k=1,2,4,8 pde-predictiveness evaluation as Sections
# 138-141/143 -- directly comparable numbers.
set -e
cd /Users/daltonjones/Documents/latent_DA

TAG=section145_localfield_maskedmlpexpand_w8_pdedistill_200ep

echo "=== [1/6] Stage 1: local_field + masked_mlp_expand (attn_window=8, expand_factor=3) + --pde-distill --pde-mutual --pde-poly-max-term-order 6, w_var=0.01/w_spatial=0.06/w_logdet=0.01/w_smooth=0.006/w_pde_distill=0.5, --amp, 200 epochs ==="
...
echo "=== [5/6] Is the pde_head actually predictive? Same k=1,2,4,8 evaluation as Sections 138-141/143 ==="
mamba run -n da_env python -c "
import h5py, numpy as np, torch
from ks_latent.models import load_autoencoder_checkpoint, load_propagator_checkpoint
